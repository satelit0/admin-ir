  ...
  if(IBQr_clave.RecordCount > 0)then
    begin
      if(IBQr_clave.FieldByName('AUTORIZAR').AsInteger = 1){or(IBQr_clave.FieldByName('R_RESERVADOS').AsInteger = 1)}then
        ...
        begin
          Result:=False;
          MS_Personalizado.MessageDlg('Usted no esta autorizado a realizar esta acción.'+#13+
          'Comuniquese con el administrador para más información.', mtInformation, [mbOK], 0);
        end;
    end
  else
    begin
...
begin
 if  not AutorizaAPP then
  begin
    MS_Personalizado.MessageDlg('Este PC no esta autorizado para ejecutar esta aplicación.', mtInformation, [mbOK], 0);
    Application.Terminate;    
  end;
end;
