...
begin
  ...
  if IBQr_Usuarios.RecordCount > 0 then
    begin
      if Mensaje = 1 then
        FPrincipal.JvBalloonHint1.ActivateHint(Control, 'No se hallarón datos con este criterio de búsqueda.', ikError, 'Error', 3000);
    end;
end;
...
begin
  try
    ...
    if Mensaje = 1 then
      FPrincipal.MS_Personalizado.MessageDlg('Usuario eliminado con éxito.', mtInformation, [mbOK], 0);
  except
    IBQr_Usuario.Free;
  end;
end;
...
begin
  try
    ...
    if Mensaje = 1 then
      FPrincipal.MS_Personalizado.MessageDlg('Usuario creado con éxito.', mtInformation, [mbOK], 0);
  except
    IBQr_Usuario.Free;
  end;
end;
...
begin
  try
    ...
    if Mensaje = 1 then
      FPrincipal.MS_Personalizado.MessageDlg('Usuario modificado con éxito.', mtInformation, [mbOK], 0);
  except
    IBQr_Usuario.Free;
  end;
end;
...
begin
  ...
  if(Trim(E_Cedula.Text) = '')then
    begin
      Result:=False;
      FPrincipal.JvBalloonHint1.ActivateHint(E_Cedula, 'Ingrese la cédula del usuario.', ikError, 'Error, campo vacio.', 3000);
    end
  else
    ...
    if(IBQr_Usuario.RecordCount > 0)then
      begin
        if((Modificar)and(Id_Usuario <> IBQr_Usuario.FieldByName('IDUSUARIO').AsInteger))or(not Modificar)then
          begin
            Result:=False;
            FPrincipal.JvBalloonHint1.ActivateHint(E_Cedula, 'La cédula ingresado ya existe para otro usuario.', ikError, 'Error en este campo.', 3000);
          end
        else
          begin
            Result:=True;
