  ...
  if(Trim(E_NombreEspecialidaMedica.Text) <> '')then
    begin
      if not ValidaEspExite(IBQr_EspecIDESPECIALIDAD.AsInteger, Trim(E_NombreEspecialidaMedica.Text), ModificarEspc)then
        ...
        begin
          JVBH_1.ActivateHint(E_NombreEspecialidaMedica, 'Este nombre de especialidad ya éxiste.', ikInformation, 'Información', 5000);
        end;
    end
  else
    begin
      FPrincipal.JvBalloonHint1.ActivateHint(E_NombreEspecialidaMedica, 'Ingrese el nombre de la especialidad médica.',ikError,'Error en este campo.',5000);
    end;
end;
...
begin
  if IBQr_Espec.Active = True then
    if IBQr_Espec.RecordCount > 0 then
      if(FPrincipal.MS_Personalizado.MessageDlg('Confirme eliminar la especialidad médica: '+
