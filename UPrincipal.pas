...
const
  ...
  NombreCentro: string = 'HOSPITAL MUNICIPAL MAIMÓN';
  DireccionCentro: string = 'C/ Padre Fantino, Municipio Maimón, Prov. Monseñor Nuel';
  E_MailCentro: string = 'E-Mail: hospital_maimon@hotmail.com';
  TelnoCentro: string = 'Teléfono: 809-551-2529'; 
...
begin
  if (C_PACIENTE_USU = 1) or (Autorizado) then
    ...
    begin
      MS_Personalizado.MessageDlg('Acceso denegado.'+#13+#13+' Usted no tiene acceso a esta opción.', mtInformation, [mbOK], 0);
    end;
end;
        ...
        begin
          ...
          if _Flogin.AutorizaAcceso(PSSDLG_Autorizar.Password) then
            begin
              ...
              'Si elimina el paciente perdera toda información referente al mismo.'+#13+
              '- Citas reservadas.'+#13+
...
begin
  ...
  OpcionBusqueda:=1;
  ME_BuscaPaciente.InsideHelpText:='Ingrese un número de registro';
...
begin
  str:=IBQr_BuscaPacienteCODIGO_PACIENTE.AsString;
  OpcionBusqueda:=4;
  ME_BuscaPaciente.InsideHelpText:='Ingrese un número de cédula';
  with IBQr_BuscaPaciente do
    begin
      Close;
      SQL.Clear;
...
begin
  ...
  if(IBQr_BuscaMedico.RecordCount > 0)then
    begin
      ...
      JvComboBox.Items.Add('Selecione el médico');
  ...
  try
    ...
    with IBQr_InsertaCita do
      begin
        ...
        Free;
        if(Mensaje = 1)then
        FPrincipal.MS_Personalizado.MessageDlg('Cita creada con éxito.',mtInformation,[mbOK],0);
      end;
  except
...
begin
  if(Trim(CBB_Medicos.Text) = '')then
    begin
      Result:=False;
      JvBalloonHint1.ActivateHint(CBB_Medicos, 'Seleccione un médico de la lista para continuar.', ikInformation, 'Información', 5000);
    end
  else
  if(CompareDate(DateOf(Calendario1.Date),DateOf(Now)) = -1)then
    begin
      ...
      'seleccione una fecha superior a la fecha actual.', ikInformation, 'Información.', 5000);
    end
  else
  if(IBQr_BuscaPaciente.Active = False)or(Id_Paciente = -1)then
    begin
      Result:=False;
      JvBalloonHint1.ActivateHint(DBG_Pacientes, 'Seleccione un paciente para continuar.', ikInformation, 'Información.', 5000);
    end
  else
          ...
          if(ValidaCitas = True)then
            begin
              ...
              if(ExcederCitas = True)then
                ...
                begin
                  MS_Personalizado.MessageDlg('Has completado el número máximo de citas por dia para este médico.'+#13+'Imposible agregar más citas a este médico en este dia.',mtInformation, [mbOK],0);
                end;
            end;
        end
      else
        begin
          JvBalloonHint1.ActivateHint(CBB_Medicos, 'Seleccione un médico de la lista para crear la cita.', ikInformation, 'Información', 5000);
        end;
    end
  else
    begin
  ...
  if IBQr_VCita.RecordCount > 0 then
    begin
      if(Modificar = False)then
        begin
          Result:=False;
          MS_Personalizado.MessageDlg('Este paciente ya tiene una cita registrada para esta fecha y para este médico', mtInformation, [mbOK],0);
        end
      else
  ...
  try
    ...
    with IBQr_Mod_Cita do
      begin
        ...
        Free;
        if Mensaje = 1 then
          MS_Personalizado.MessageDlg('Cita modificada éxcitosamente.', mtInformation, [mbOK],0);
      end;
  except
  ...
  try
    ...
    with IBQr_ElimCita do
      begin
        ...
        Free;
        if(Mensaje = 1)then
          FPrincipal.MS_Personalizado.MessageDlg('Cita eliminada éxitosamente.',mtInformation,[mbOK],0);
      end;
  except
  end;
end;
...
begin
  ...
  PN_UltimaSesion.Caption:='Última Sesión Fecha: '+DateToStr(UltimaFechaSesionUsu)+' Hora: '+TimeToStr(UltimaHoraSesionUsu);
...
begin
    ...
    begin
      ...
      if(IBQr_BuscaPaciente.RecordCount < 1)then
        begin
          DBG_PacientesCellClick(nil);
          if Mensaje = 1 then
            JvBalloonHint1.ActivateHint(ME_BuscaPaciente, 'No se hallarón datos para este criterio de búsqueda.', ikInformation, 'Información.', 2000);
        end
      else
        begin
 ...
 {   end
    ...
    begin
      JvBalloonHint1.ActivateHint(ME_BuscaPaciente, 'Ingrese un criterio de búsqueda.', ikError, 'Error campo vacio', 5000);
    end;}
end;
...
begin
  ...
  if(leCadINI('SUCONF', 'BX', '0')='1')then
    begin
      BusquedaExacta:=True;
      LB_1.Caption:='(Modo búsqueda Exacto Activo.)';
    end
  else
    begin
      BusquedaExacta:=False;
      LB_1.Caption:='(Modo búsqueda Aproximado Activo.)';
    end;
...
begin
  str:=IBQr_BuscaPacienteCODIGO_PACIENTE.AsString;
  OpcionBusqueda:=6;
  ME_BuscaPaciente.InsideHelpText:='Ingrese un número de seguro social';
  with IBQr_BuscaPaciente do
    begin
      Close;
      SQL.Clear;
...
begin
  str:=IBQr_BuscaPacienteCODIGO_PACIENTE.AsString;
  OpcionBusqueda:=7;
  ME_BuscaPaciente.InsideHelpText:='Ingrese un número de pasaporte';
  with IBQr_BuscaPaciente do
    begin
      Close;
      SQL.Clear;
          ...
          if(ExcederCita = 0)and(CitasRestantes > 0)then
            begin
              ...
              if (CitasRestantes <= 3)and(Mensaje = 1) then
                JvBalloonHint1.ActivateHint(LB_26,'Sólo quedan disponibles: '+IntToStr(CitasRestantes),ikWarning,'Precaución.',5000);
            end
          else
          if (ExcederCita = 0)and(CitasRestantes = 0) then
            begin
...
begin
  str:=IBQr_BuscaPacienteCODIGO_PACIENTE.AsString;
  OpcionBusqueda:=8;
  ME_BuscaPaciente.InsideHelpText:='Ingrese un número teléfono';
  with IBQr_BuscaPaciente do
    begin
      Close;
      SQL.Clear;
    ...
    begin
      ...
      with _FDatosMicelaneos do
        begin
          ...
          Caption:='Reporte de Consultas por Diagnóstico y Edad.';
          ShowModal;
          Free;
        end;
    end
     ...
     _FDatosMicelaneos:=TFDatosMiscelaneos.Create(Self);
      with _FDatosMicelaneos do
        begin
          ...
          Caption:='Reporte de Citas por Médico.';
          GB_10.Caption:='Lista de Médicos.';
          LB_24.Caption:='Nombre Médico';
          ShowModal;
          Free;
        end;
    end
      ...
      Autorizado:=False;
       ...
       with _FDatosMicelaneos do
          begin
            ...
            BT_MostrarCitasMedico.Default:=True;
            BT_MostrarCitasMedico.Caption:='&Enviar...';
            Caption:='Reporte de Citas Especialidad Médica.';
            GB_10.Caption:='Lista de Especialidades Médica.';
            LB_24.Caption:='Nombre Especialidad Médica';
            ShowModal;
            Free;
          end;
    end
...
begin
  try
    try
      ...
      if Mensaje = 1 then
        MS_Personalizado.MessageDlg('Paciente eliminado con éxito.', mtInformation, [mbOK], 0);
    finally
...
begin
  IBQr_Estado:=TIBQuery.Create(Self);
    ...
    if Mensaje = 1 then
      begin
        MS_Personalizado.MessageDlg('Acción realizada con éxito.', mtInformation, [mbOK], 0);
      end;
...
begin
  ...
  _Fconf.RG_BackupRestore.Caption:='Opciones de restauración';
...
begin
  if Trim(IBQr_BuscaPacienteTELNO_TRABAJO.AsString) <> '' then
    if Trim(IBQr_BuscaPacienteTRABAJO_EXT.AsString) <> '' then
      ...
      '  Extención: '+IBQr_BuscaPacienteTRABAJO_EXT.AsString
    else
      Text:=FormatMaskText('(999)999-9999;0; ', IBQr_BuscaPacienteTELNO_TRABAJO.AsString)
  else
    Text:='';
